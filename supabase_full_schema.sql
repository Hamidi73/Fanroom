...
-- ─────────────────────────────────────────────────────────────────────────
-- 0008  follow chat notice  (announce a new follow in the room's chat)
-- ─────────────────────────────────────────────────────────────────────────
-- toggle_follow gains a room param and posts a '[follow]' marker message on a
-- new follow (RoomChat renders it as a system line) — once per follower/room.
-- Chat-only; on-screen alerts stay the paid feature.
...
begin
  ...
  if exists (select 1 from public.follows f where f.creator_id = p_creator_id and f.follower_id = uid) then
    ...
    if p_room_id is not null
       ...
       and not exists (select 1 from public.messages m where m.room_id = p_room_id and m.user_id = uid and m.body = '[follow]')
    then
      insert into public.messages (room_id, user_id, body, highlight) values (p_room_id, uid, '[follow]', false);
    end if;
  end if;
  return json_build_object(
    'followers', (select count(*) from public.follows f where f.creator_id = p_creator_id),
