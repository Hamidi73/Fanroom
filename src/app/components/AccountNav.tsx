...
export function AccountNav() {
  ...
  // Close the account menu on outside tap/click or Escape — a tap-driven menu
  // works on touch devices (the old hover-only menu was unreachable on mobile).
  useEffect(() => {
    if (!menuOpen) return;
  ...
  if (!name) {
    return (
      <span className="flex items-center gap-3">
        {/* "Rooms" is redundant with the search box on small screens — hidden
            there so the top-nav search stays usable. */}
        <Link href="/rooms" className="hidden text-sm font-medium text-muted no-underline hover:text-ink-foreground sm:inline">
          Rooms
        </Link>
  ...
  // Account menu opens on tap/click (works on touch — the old hover-only menu
  // was unreachable on phones, stranding Sign out). Closes on outside tap/Esc.
    ...
    <span className="flex items-center gap-3">
      ...
      <span ref={menuRef} className="relative">
        <button
          ...
          className="flex items-center gap-2 rounded-lg px-2 py-1.5 text-sm font-medium text-ink-foreground transition hover:bg-surface-2"
        >
          ...
          <svg width="10" height="10" viewBox="0 0 16 16" fill="none" aria-hidden="true" className={`text-muted transition ${menuOpen ? "rotate-180" : ""}`}>
        ...
        {menuOpen && (
          <div className="absolute right-0 top-full z-50 mt-2 w-52">
            <div role="menu" className="overflow-hidden rounded-xl border border-line bg-ink p-1.5 shadow-2xl shadow-black/60">
              ...
              <Link href="/rooms/new" onClick={close} className={itemClass}>
                <MenuIcon path="M1.5 4.5h9v7h-9zM10.5 7.5 14.5 5v6l-4-2.5" /> Host a room
              </Link>
              <Link href="/rooms" onClick={close} className={itemClass}>
                <MenuIcon path="M7 12A5 5 0 1 0 7 2a5 5 0 0 0 0 10Zm3.7-1.3L14 14" /> Browse rooms
              </Link>
              {isAdmin && (
                <Link href="/admin" onClick={close} className={`${itemClass} font-bold text-accent-soft`}>
                  <MenuIcon path="M8 1.5 13.5 3.5v4c0 3.2-2.3 5.8-5.5 7-3.2-1.2-5.5-3.8-5.5-7v-4L8 1.5Z" /> Admin
                </Link>
              )}
              <div className="my-1 border-t border-line" />
              <button onClick={() => { close(); void signOut(); }} className={`${itemClass} text-muted`}>
